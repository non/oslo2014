...
object FlatMap {
  ...
  object Show {
    def apply[A](f: A => String) =
      new Show[A] { def show(a: A): String = f(a) }
  }
  ...
  implicit val IntHasShow = Show[Int](_.toString)
  implicit val DoubleHasShow = Show[Double](_.toString)
  implicit val BooleanHasShow = Show[Boolean](b => if (b) "x" else ".")

  implicit def optionHasShow[A: Show] =
    Show[Option[A]](_.map(_.show).getOrElse("-"))

  implicit def listHasShow[A: Show] =
    Show[List[A]](_.map(_.show).mkString("[", ",", "]"))

  implicit def streamHasShow[A: Show] =
    Show[Stream[A]](_.headOption.map("[" + _.show + ",...]").getOrElse("[]"))

  /**
   * StarRig[A] is a Rig[A] that also has an asteration operator: kstar.
   * 
   * Laws:
   * 1. a.kstar = 1 + a * a.kstar = 1 + a.kstar * a
   */
  ...
  implicit class StarRigOps[A: StarRig](a: A) {
    def kstar(): A = macro Ops.unop[String]
    def kplus(): A = macro Ops.unop[String]
  }
  ...
  implicit def matrixHasShow[A: Show] =
    new Show[Matrix[A]] {
      def show(m: Matrix[A]): String = {
        val n = m.dim.n
        val lines = Array.fill(n)("")
        cfor(0)(_ < n, _ + 1) { x =>
          cfor(0)(_ < n, _ + 1)(y => lines(y) += m(x, y).show + " ")
  ...
  implicit def exprHasShow[A: Show] =
    new Show[Expr[A]] {
  ...
  /**
   * Weight represents a finite quantity between zero and infinity.
   */
  ...
  implicit def tropicalHasShow[A: Show] = new Show[Weight[A]] {
    def show(t: Weight[A]) = t match {
      case Finite(a) => a.show //Show[A].show(a)
      case Infinity => "∞"
    }
  }

  implicit def tropicalHasOrder[A](implicit ord: Order[A]) = new Order[Weight[A]] {
    def compare(x: Weight[A], y: Weight[A]) = (x, y) match {
  ...
  implicit def shortestPathHasKleene[A, B](implicit rig: Rig[Weight[A]], ord: Order[Weight[A]], kb: Kleene[B]) =
  ...
  implicit def languageHasShow[W: Show] = new Show[Language[W]] {
    def show(l: Language[W]) = l.wss.show
  }
  ...
  def pathExample() {
    ...
    val weighted: Matrix[Weight[Int]] = {
      val m = ArrayMatrix(Array.fill(dim.n * dim.n)(Weight.inf[Int]))
    ...
    val annotated = Matrix[ShortestPath[Int, Expr[Edge]]] { (x, y) =>
      weighted(x, y) match {
        ...
        case Infinity => ShortestPath(Infinity, Rig[Expr[Edge]].zero)
    ...
    val langed = Matrix[ShortestPath[Int, Language[Edge]]] { (x, y) =>
      weighted(x, y) match {
        case Infinity => ShortestPath(Infinity, Rig[Language[Edge]].zero)
    ...
    val leastCostExprs: Matrix[Weight[Int]] =
      costExprs.kstar.map(a => evalExpr(a)(Weight.apply))
